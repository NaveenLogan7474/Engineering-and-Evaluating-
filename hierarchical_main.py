...
from modelling.hierarchical_data_model import HierarchicalData
...
def get_hierarchical_data_object(X: np.ndarray, df: pd.DataFrame):
    """Create a HierarchicalData object with the given data."""
    return HierarchicalData(X, df)


def perform_hierarchical_modelling(data: HierarchicalData, df: pd.DataFrame, name):
    """Perform hierarchical multi-label classification."""
    print(f"\n===== Processing {name} with Hierarchical Classification =====")
    
    # Skip if data processing resulted in insufficient samples
    if data.get_type2_train_data()[0] is None:
    ...
    model.train(data)
    X_test, _ = data.get_type2_test_data()
    model.predict(X_test)
    model.print_results(data)
    
    # Save the model
    model_path = f"models/{name}_hierarchical_model.pkl"
    try:
...
if __name__ == '__main__':
    print("========== DESIGN DECISION 2: HIERARCHICAL MODELLING ==========")
    ...
    for name, group_df in grouped_df:
        ...
        data = get_hierarchical_data_object(X, group_df)
        
        # Perform hierarchical classification
        model = perform_hierarchical_modelling(data, group_df, name) 